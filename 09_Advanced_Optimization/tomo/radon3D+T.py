...
img = np.load('./pytorch_radon/shepp_phantom.npy')
img = gaussian(img, 1)
...
sino_target = ray_transform(target, phi_rad_target, theta_rad_target, psi_rad_target, translation_target)
...
phi_rad_model += th.as_tensor(np.random.uniform(-3.14, 3.14, phi_rad_target.shape[0]) / 50,
                              device=device)
fig, ax = plt.subplots()
ax.scatter(np.arange(len(phi_rad_model)), phi_rad_model.detach().cpu().numpy().squeeze())
